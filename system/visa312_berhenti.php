...
if (isset($_GET['m'])) {
    ...
    $data = "SELECT * FROM visa312 WHERE id ='$id' ";
    ...
    $visa = $row_data['visa'];
    $no_rptka = $row_data['no_rptka'];
    $jabatan = $row_data['jabatan'];
    $tgl_terbit = $row_data['tgl_terbit'];
    $jangka_visa = $row_data['jangka_visa'];
    $no_imta = $row_data['no_imta'];
    $exp_kitas = $row_data['exp_kitas'];
    $notif = $row_data['notif'];
    $ket = $row_data['ket'];
    $tgl_input = $row_data['tgl_input'];
    $input_by = $row_data['input_by'];


    $query = "INSERT INTO riwayat_visa312 VALUE ('','$nama_pt', '$passport','$visa', '$no_rptka', '$jabatan', '$tgl_terbit', '$jangka_visa', '$no_imta', '$exp_kitas', '$notif', '$ket', '$tgl_input', '$input_by', '$tgl_stop', '$stop_by')";
    $result = mysqli_query($connection, $query);

    $hapus = "DELETE FROM visa312 WHERE id = '$id' ";
    ...
    header("location:../beranda/report?page=visa312");
}
