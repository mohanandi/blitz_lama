<div class="container" align="center" style="margin-top:101px;">
  <h2> DATA VISA 211 AKTIF <?php echo $nama_pt ?> </h2>
  ...
  <div class="row">
    <div class="btn-group dropright pull-right">
      ...
      <div class="dropdown-menu">
        ...
        <div class="col-xs-12 col-md-12"><input type="submit" value="by date" class="btn btn-link" style="color: black;" tabindex="7" data-toggle="modal" data-target="#date"></div>
...
<div class="scrollable-table col-md-11" style="background-color: #f5f5f5; margin-left: 55px;">
  ...
  <table class="table table-hover table-bordered results row-border order-column display">
    ...
    $query = "SELECT * FROM riwayat_visa211 WHERE nama_pt='$nama_pt'";
    $vis = mysqli_query($connection, $query);
    if (mysqli_num_rows($vis) != 0) {
      while ($row_vis = mysqli_fetch_array($vis)) {

        $no = 0;

        $passport = $row_vis['passport'];
    ?>

        <tbody>
          <?php
          $query = "SELECT * FROM riwayat_data WHERE nama_pt='$nama_pt' AND passport = '$passport' ";
          ...
          <tr>
            ...
            <?php
            $stop_by = $row_vis['stop_by'];
            $tgl_stop = $row_vis['tgl_stop'];
            ?>
            <?php
            ...
            <?php
            $query = "SELECT * FROM riwayat_visa211_1 WHERE nama_pt='$nama_pt' AND passport='$passport'";
            $vis = mysqli_query($connection, $query);
            if (mysqli_num_rows($vis) == 0) {
            ?>
            ...
            $query = "SELECT * FROM riwayat_visa211_2 WHERE nama_pt='$nama_pt' AND passport='$passport'";
            $vis = mysqli_query($connection, $query);
            if (mysqli_num_rows($vis) == 0) {
            ?>
            ...
            $query = "SELECT * FROM riwayat_visa211_3 WHERE nama_pt='$nama_pt' AND passport='$passport'";
            $vis = mysqli_query($connection, $query);
            if (mysqli_num_rows($vis) == 0) {
            ?>
              <td><?php echo "Belum Ada"; ?></td>
            ...
            $query = "SELECT * FROM riwayat_visa211_4 WHERE nama_pt='$nama_pt' AND passport='$passport'";
            $vis = mysqli_query($connection, $query);
            if (mysqli_num_rows($vis) == 0) {
            ?>
              <td><?php echo "Belum Ada"; ?></td>
            ...
            <td><?php echo $tgl_stop; ?></td>
            <td><?php echo $stop_by; ?></td>
