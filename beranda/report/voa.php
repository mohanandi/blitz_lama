...
function seluruh($tanda, $nama_user)
{
  ...
  for ($i = 0; $i < count($tanda); $i++) {
    $query = "SELECT * FROM voa where id = $tanda[$i]";
    $result = mysqli_query($connection, $query);
    ...
    $id_singgah = $row['passport'];
    $pt_singgah = $row['nama_pt'];
    $query_person = "SELECT * FROM data WHERE nama_pt='$pt_singgah' AND passport='$id_singgah' ";
...
?>
    ...
    <tr>
      <td><?php echo $i + 1;  ?></td>
      <td>
        <a href="../../system/voa_berhenti.php?m=<?php echo $row['id']; ?>&input=<?php echo $nama_user; ?>" class="badge badge-danger float-left ml-1" onclick="return confirm('Apakah Anda Yakin Akan Memberhentikan User Ini?');">Berhentikan</a>
      </td>
