<div class="container" style="margin-top:101px;">
  ...
  function seluruh($tanda, $nama_user)
  {
    ...
    for ($i = 0; $i < count($tanda); $i++) {
      $query = "SELECT * FROM visa_lain where id = $tanda[$i]";
      $result = mysqli_query($connection, $query);
  ...
  ?>
      ...
      <tr>
        <td><?php echo $i + 1;  ?></td>
        <td>
          <a href="../../system/visalain_berhenti.php?m=<?php echo $row['id']; ?>&input=<?php echo $nama_user; ?>" class="badge badge-danger float-left ml-1" onclick="return confirm('Apakah Anda Yakin Akan Memberhentikan User Ini?');">Berhentikan</a>
        </td>
