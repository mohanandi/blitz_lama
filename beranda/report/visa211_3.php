<div class="container" style="margin-top:101px;">
  ...
  <div class="row">
    ...
    while ($row = mysqli_fetch_assoc($result)) {
      ...
      $visa_lanjut = "SELECT * FROM visa211_4 WHERE passport = '$singgah_passport'";
  ...
  function seluruh($tanda, $nama_user)
  {
    ...
    for ($i = 0; $i < count($tanda); $i++) {
      $query = "SELECT * FROM visa211_3 WHERE id = $tanda[$i]";
      $result = mysqli_query($connection, $query);
  ...
  ?>
      ...
      <tr>
        ...
        <?php
        $visa211_1 = "SELECT * FROM visa211_4 WHERE passport = '$passport' ";
        ...
        ?>
          <td>
            <a href="../../system/visa211_3_berhenti.php?m=<?php echo $row['id']; ?>&input=<?php echo $nama_user; ?>" class="badge badge-danger float-left ml-1" onclick="return confirm('Apakah Anda Yakin Akan Memberhentikan User Ini?');">Berhentikan</a>
            <a href="../data?passport=<?php echo $row['passport']; ?>&page=tambah_visa211_4" class="badge badge-danger float-left ml-1" onclick="return confirm('Apakah Anda Yakin Akan Extend Visa 211-4 User Ini?');">Perpanjang Visa211-4</a>
          </td>
