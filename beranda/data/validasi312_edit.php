...
<div class="container" style="margin-top:101px;">
  ...
  <table id="tab" class="table table-hover table-bordered" style="width:60%;margin-top:50px;">
    <form method="post" action="index.php?page=tambah_visa312_edit">
      <input type="hidden" class="form-control" name="passport" value="<?php echo $passport; ?>">
      <input type="hidden" class="form-control" name="rptka_baru" value="<?php echo $rptka_baru; ?>">
      <input type="hidden" class="form-control" name="rptka_lama" value="<?php echo $rptka_lama; ?>">
  ...
  <table id="tab" class="table table-hover table-bordered" style="width:60%;margin-top:50px;">
    <form method="post" action="../../system/visa312_edit.php">
      <input type="hidden" class="form-control" name="passport" value="<?php echo $passport; ?>">
      <input type="hidden" class="form-control" name="rptka_lama" value="<?php echo $rptka_lama; ?>">
      <input type="hidden" class="form-control" name="rptka_baru" value="<?php echo $rptka_baru; ?>">
      <input type="hidden" class="form-control" name="no_imta" value="<?php echo $no_imta; ?>">
      ...
      <tr>
        ...
        <td><select name="jabatan" id="jabatan" class="form-control" required>
            <?php
            if ($rptka_baru == $rptka_lama) {
              $query_jabatan = "SELECT jabatan FROM visa312 WHERE passport = '$passport'";
              $sql_jabatan = mysqli_query($connection, $query_jabatan);
              $tanda_jabatan = mysqli_fetch_array($sql_jabatan);
            ?>
              <option selected><?php echo $tanda_jabatan['jabatan']; ?></option>
            <?php
            }
            ?>

            <?php
            $query = "SELECT * FROM jabatan_rptka WHERE nama_pt = '$nama_pt' && no_rptka = '$rptka_baru' ";
            $result = mysqli_query($connection, $query);
            while ($row_jabatan = mysqli_fetch_array($result)) {
            ?>

              <?php if ($row_jabatan['jumlah'] - $row_jabatan['terpakai'] != 0) { ?>
                <option><?php echo $row_jabatan['jabatan']; ?></option>
              <?php } ?>
            <?php } ?>
          </select></td>
      </tr>
